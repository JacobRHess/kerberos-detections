...
kerbdetect is a personal detection-engineering lab with no hosted deployment to attack. If you
...
- TLS verification to Splunk is off by default for the lab's self-signed localhost cert and
  is turned on with `KD_SPLUNK_VERIFY=true` for a real, trusted endpoint.
