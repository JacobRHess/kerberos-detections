...
from kerbdetect import report as report_mod
from kerbdetect import schema
from kerbdetect.evtx import CaptureError
...
from kerbdetect.replay import ReplayError, check_model_for_replay, format_outcome, run_replay
from kerbdetect.splunk import SplunkClient, SplunkError, config_from_env
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="kerbdetect", description=__doc__)
    ...
    cap.add_argument("exports", nargs="+", type=Path, help="stage-NN-*.kerbdetect.xml files")
    ...
    rep.add_argument("--url", help="Splunk REST url ($KD_SPLUNK_URL)")
    rep.add_argument("--hec-url", help="Splunk HEC url ($KD_SPLUNK_HEC_URL)")
    rep.add_argument("--user", help="admin username ($KD_SPLUNK_USER)")
    rep.add_argument("--password", help="admin password ($KD_SPLUNK_PASSWORD)")
    rep.add_argument("--index", help="target index ($KD_SPLUNK_INDEX)")
    ...
    new.add_argument(
        ...
        help="e.g. security=4769 (repeatable)",
    )
    return parser
...
def _command_new(model: Model, args: argparse.Namespace) -> int:
    ...
    for chunk in args.slices:
        ...
        if source not in SOURCES or not codes_raw:
            raise CommandError(f"--slice must look like security=4769, got {chunk!r}")
        try:
    ...
    print(f"next: write rules/{args.id}.spl, then slice fixtures with 'kerbdetect capture'")
    return 0
