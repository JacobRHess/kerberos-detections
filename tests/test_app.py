...
from kerbdetect.app import build_app, render_savedsearches
from kerbdetect.model import ModelError, check_rules, load_model
...
def test_build_app_empty_model(tmp_path):
    ...
    assert (out / "default" / "data" / "ui" / "views" / "kerbdetect_coverage.xml").is_file()
...
def test_build_app_renders_saved_search(tmp_path):
    ...
    assert f"[kerbdetect - {DETECTION_ID}]" in conf
    assert f"search = index=kerbdetect {DEFAULT_RULE}" in conf
    assert 'mitre_technique="T1558.003"' in conf
    assert 'kerbdetect_stage="01-credential-access"' in conf
...
def test_app_conf_shape(tmp_path):
    ...
    assert "label = kerbdetect" in conf
