...
def test_full_model_loads(make_root):
    ...
    assert detection.stage == "01-credential-access"
    assert detection.title == "Kerberoasting via RC4-downgraded service-ticket requests"
    assert detection.attack == ("T1558.003",)
    assert detection.slice == {"security": (4769,)}
    ...
    assert len(model.stage_detections("01-credential-access")) == 1
...
def test_stage_resolution(make_root):
    model = load_model(make_root())
    assert model.stage("01-credential-access").id == "01-credential-access"
    assert model.stage("01").id == "01-credential-access"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda t: t.replace("- id: 01-credential-access", "- id: bad slug"), "stage id"),
        (lambda t: t.replace("techniques: [T1558.003]", "techniques: []"), "must list techniques"),
        (
            lambda t: t.replace("techniques: [T1558.003]", "techniques: [X1003]"),
            "invalid technique",
        ),
        (
            lambda t: t.replace(
        ...
        (
            lambda t: t.replace(
                "title: Kerberoasting via RC4-downgraded service-ticket requests", "title: ''"
            ),
            "title",
        ),
        (
        ...
        (
            lambda t: t.replace("stage: 01-credential-access", "stage: 99-nowhere"),
            "unknown stage",
        ),
        (lambda t: t.replace("attack: [T1558.003]", "attack: []"), "attack techniques"),
        (lambda t: t.replace("attack: [T1558.003]", "attack: [nope]"), "invalid technique"),
        (lambda t: t.replace("      security: [4769]", "      security: [true]"), "invalid code"),
        (
            lambda t: t.replace("      security: [4769]", "      security: []"),
            "must list event codes",
        ),
        (
            lambda t: t.replace("      security: [4769]", "      netflow: [4769]"),
            "slice source",
        ),
        (
...
def test_check_rule_contract(make_root):
    ...
    rule_path.write_text("| stats count by ServiceName", encoding="utf-8")
    with pytest.raises(ModelError, match="must not start with a pipeline"):
        check_rule(rule_path)

    rule_path.write_text("search EventCode=4769", encoding="utf-8")
    with pytest.raises(ModelError, match="bare search terms"):
        check_rule(rule_path)

    rule_path.write_text("EventCode=4769 index=windows", encoding="utf-8")
    with pytest.raises(ModelError, match="'index'"):
        check_rule(rule_path)

    rule_path.write_text("EventCode=4769 earliest=-30m", encoding="utf-8")
    ...
    rule_path.write_text(
        'EventCode=4769 note="talks about index= and earliest=" | top ServiceName',
        encoding="utf-8",
    )
