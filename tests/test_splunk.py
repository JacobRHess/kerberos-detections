...
from kerbdetect import splunk as splunk_mod
from kerbdetect.schema import SECURITY
from kerbdetect.splunk import (
    DEFAULT_HEC_URL,
    DEFAULT_PASSWORD,
    DEFAULT_REST_URL,
    DEFAULT_USER,
...
def bootstrap_ok_responses() -> list[FakeResponse]:
    # One sourcetype (Security), so one props GET + one props POST update.
    responses = [FakeResponse(200, {"entry": []})]
    for _ in range(len(splunk_mod.SOURCETYPES)):
...
def test_config_from_env(monkeypatch):
    monkeypatch.setenv("KD_SPLUNK_URL", "https://splunk.example:8089")
    monkeypatch.setenv("KD_SPLUNK_HEC_URL", "https://splunk.example:8088")
    monkeypatch.setenv("KD_SPLUNK_USER", "root")
    monkeypatch.setenv("KD_SPLUNK_PASSWORD", "hunter2")
    monkeypatch.setenv("KD_SPLUNK_INDEX", "other")
    config = config_from_env()
...
def test_config_verify_from_env(monkeypatch, value):
    monkeypatch.setenv("KD_SPLUNK_VERIFY", value)
...
def make_event() -> dict[str, Any]:
    return {
        "sourcetype": SECURITY,
        "_time": "2026-08-14T15:20:01.123456+00:00",
...
def test_hec_batch_shape():
    body = hec_batch(
        [make_event()], run_tag="abc-det-attack", index="kerbdetect", source="fixture.json"
    )
    (line,) = body.splitlines()
    record = json.loads(line)
    assert record["index"] == "kerbdetect"
    assert record["source"] == "fixture.json"
    assert record["sourcetype"] == SECURITY
    assert record["host"] == "DC01.range.lab"
    expected_epoch = datetime.fromisoformat("2026-08-14T15:20:01.123456+00:00").timestamp()
    assert record["time"] == pytest.approx(expected_epoch)
    assert record["event"]["kd_run"] == "abc-det-attack"
    assert record["event"]["EventCode"] == 4769
...
def test_bootstrap_creates_missing_objects():
    responses = [
        FakeResponse(404),
        FakeResponse(200, {"entry": [{"name": "kerbdetect"}]}),
        FakeResponse(404),
...
def test_oneshot_search_parses_rows():
    ...
    client.bootstrap()
    rows = client.oneshot_search("kerbdetect | stats count", earliest="a", latest="b")
    assert rows == [{"count": "2"}]
