...
from kerbdetect.capture import SliceError, run_capture, slice_events, write_fixture
...
NO_SLICE_YAML = DEFAULT_YAML.replace("    slice:\n      security: [4769]\n", "")
...
def test_slice_events_filters_by_channel_and_code():
    events = [
        ...
        {"sourcetype": schema.SECURITY, "EventCode": 4768},
        {"sourcetype": schema.SECURITY, "EventCode": 4769},
        {"sourcetype": schema.SECURITY, "EventCode": 4769},
        {"sourcetype": schema.SECURITY, "EventCode": 4672},
    ]
    kept = slice_events(events, {"security": (4769,)})
    assert [e["EventCode"] for e in kept] == [4769, 4769]
...
def test_attack_capture_explicit_stage(make_root, sample_capture):
    root = make_root()
    model = load_model(root)
    for ref in ("01", "01-credential-access"):
...
def test_attack_capture_unknown_stage_attr(make_root, sample_capture, tmp_path):
    ...
    other.write_text(
        sample_capture.read_text(encoding="utf-8").replace('stage="01"', 'stage="99"', 1),
        encoding="utf-8",
    )
...
def test_schema_violation_in_capture_fails(tmp_path, sample_capture):
    ...
    degraded.write_text(
        sample_capture.read_text(encoding="utf-8").replace(
            '<Data Name="TicketEncryptionType">0x17</Data>', "", 1
        ),
        encoding="utf-8",
    )
    with pytest.raises(schema.SchemaError, match="TicketEncryptionType"):
